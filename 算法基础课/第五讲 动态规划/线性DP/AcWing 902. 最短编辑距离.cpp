...
// 增：在 a[i] 后面添加一个 b[j]，让a[1..i] 匹配 b[1..j-1]：f[i,j-1] + 1
// 删：把 a[i] 删掉，让 a[1..i-1] 匹配 b[1..j]：f[i-1,j] + 1
// 换：把 a[i] 改成 b[j]，让 a[1..i-1] 匹配 b[1..j-1]：f[i-1,j-1] if a[i]==b[j] else f[i-1,j-1]+1
int main() {
  scanf("%d%s", &n, a + 1);
  scanf("%d%s", &m, b + 1);
